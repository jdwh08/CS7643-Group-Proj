# src/training/prithvi/train_hand.py
#####################################################################
# Flood Segmentation Project [Vision Transformer Transfer Learning with Prithvi]
#####################################################################

### ABOUT
"""Training a Prithvi Vision Transformer model on Sen1Floods11 (hand labels)."""
...
ALL_BACKBONE_NAMES = ("prithvi_eo_v1_100", "prithvi_eo_v2_300_tl")
...
BACKBONE = ALL_BACKBONE_NAMES[1]
BACKBONE_INDICES = ALL_BACKBONE_INDICES[BACKBONE]
DECODER = ALL_DECODERS[1]  # unet, upernet, fcn
DECODER_CHANNELS = [1024, 512, 256, 128] if ALL_DECODERS[1] == "UNetDecoder" else 512
HEAD_DROPOUT = 0.05
WEIGHT_DECAY = 0.01
LR = 1.0e-4
...
#     "eta_min": LR / 1e4,
# }
ES_PATIENCE = 10  # NOTE: ES auto disabled if using cyclic lr scheduler
ES_THRESH = 1e-3
...
class PrithviHandConfig:
    ...
    decoder_channels: Any = DECODER_CHANNELS
    ...
    es_patience: int = (
        ES_PATIENCE  # NOTE: ES auto disabled if using cyclic lr scheduler
    )
...
class PrithviHandTrainer:
    """End-to-end training pipeline for hand-labeled Sen1Floods11."""

    def __init__(
        self, config: PrithviHandConfig, config_path: Path | None = None
    ) -> None:
        self.config = config
    ...
    def _build_datamodule(self) -> Sen1Floods11S2HandDataModule:
        return Sen1Floods11S2HandDataModule(
...
def load_config(config_path: Path | None = None) -> dict[str, Any]:
    """Load YAML configuration for the hand trainer."""
...
def main(config_path: Path | None = None) -> Path:
    ...
    trainer_config = PrithviHandConfig.from_overrides(raw_config.get("settings"))
    trainer = PrithviHandTrainer(trainer_config, config_path=config_path)
    return trainer.run()
